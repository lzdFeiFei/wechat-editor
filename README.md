...
- `/` 首页介绍 + 功能入口
- `/templates` 模板管理列表页（模板 CRUD + 悬浮预览 + 跳转详情）
- `/templates/[id]` 模板详情配置页（三栏调参 + HTML 分析导入）
- `/style-builder` 元素样式配置页（样式库管理 + 排版参考提取导入）
...
- 新增元素样式资产页 `/style-builder`
- 支持从“排版参考”HTML 中按元素类型提取样式（`h1/h2/h3/p/li/blockquote/img/hr`）
- 仅导入参考文档中实际出现且可提取到字段的元素类型
- 命名规则：`参考文档名-元素类型`，同名自动重命名为 `-2/-3/...`
- 导入后提供成功项、重命名项、警告项反馈
